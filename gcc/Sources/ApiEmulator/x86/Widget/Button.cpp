...
void Button::paintEvent()
{
    ...
    if(m_icon) {
        ...
        if(style().alighIcon() & (1 << 1))     // x center
            px = rect().w()/2 - m_icon->width()/2;

        if(style().alighIcon() & (1 << 2))     // y center
            py = rect().h()/2 - m_icon->height()/2;

        drawImage(realRect().x() + px + style().iconOffset().x(),
                  realRect().y() + py + style().iconOffset().y(), m_icon);
    }
