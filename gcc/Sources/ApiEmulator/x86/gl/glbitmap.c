...
void glCtxDrawOptionalBitmap(GLContext *ctx, int x, int y,
                             int bit_offset_x, int bit_offset_y, int x2, int y2,
                             unsigned char alpha, int w, int h, int depth, const void *bitmap)
{
    ...
    if(cx+max_w-offset_x > ctx->clip_x2)
        max_w = ctx->clip_x2-cx + offset_x;

    if(cy+max_h-offset_y > ctx->clip_y2)
        max_h = ctx->clip_y2-cy + offset_y;
            ...
            switch(depth)
            {
                case 16:  // 16->16 no alpha blend
                    if(alpha == 0)
                        ...
                        for_each {
                            glDrawPixel16_16ca(ctx, cx, cy, *(bp16 + (_y*w+_x)), _alpha);
                        }
                    }
                    break;
                    ...
                    if(ctx->attr & GL_ALPHA_TEST) {
                        if(alpha != 0) {
                            for_each {
                                ...
                                glDrawPixel16_32ca(ctx, cx, cy, c, _alpha);
                            }
                        } else {
                            for_each {
                                GLColor c = *(bp32 + (_y*w+_x));
                                ...
                                //{
                                    if(a != 0) {
                                        if(a != 255) {
                                            glDrawPixel16_32ca(ctx, cx, cy, c, a);
                                        } else {
            ...
            switch(depth)
            {
                case 16:
                    if(alpha == 0)
                        ...
                        for_each glDrawPixel32_16ca(ctx, cx, cy, *(bp16 + (_y*w+_x)), _alpha);
                    break;
