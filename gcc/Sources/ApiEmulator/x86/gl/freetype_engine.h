...
{
    const void *str;
    const void *last_breaked_char_pos;
    int charscount;
    int last_breaked_char;
    int width;
    int height;
    int top;
    char strend;
...
enum
{
    ...
    FT_TEXT_CRBREAK  = (1 << 9),
    FT_TEXT_SENTENCEBREAK  = (1 << 10),
    FT_TEXT_NOCALC_HCENTER = (1 << 11)
};
...
uint32_t ftStringMetrics(fte_info *fti, const void *str, ft_metrics *metr, int32_t max_width,
                         int32_t max_leter_cnt, int flags, wchar_t (*conv)(const void **));
...
uint32_t ftStringMetrics(int font, const void *str, ft_metrics *metr, int32_t max_leter_cnt, wchar_t (*conv)(const void **));
