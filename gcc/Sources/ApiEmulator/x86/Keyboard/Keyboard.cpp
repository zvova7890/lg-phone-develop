...
void KeyboardHelper::createKeyboard(const std::vector<std::vector<const char *> > &kbd_layout)
{
    ...
    Image *kbd_c = res? &res->image("kbd_c") : 0;
    Image *kbd_up = res? &res->image("kbd_up") : 0;
    ...
    for(const std::vector<const char *> &vec : kbd_layout) {
        ...
        float cc_btn = width + (width*8 / 100);
        float xpos = 1;
        Button *b = 0;

        if(i == 2) {
            width = float(rect().w() - 2 - (cc_btn*2)) / vec.size();

            b = new Button(Rect(xpos, hstep, cc_btn, 30), this, "up");
            caseTypeSwitcher = b;
            b->setIcon(kbd_up);
            b->setActive(false);
        ...
        if(i == 2) {
            b = new Button(Rect(xpos, hstep, cc_btn, 30), this, "clean");
            b->setIcon(kbd_c);
            b->setTextVisible(false);
            //b->setMovable(false);
    ...
    Ololo ks[] = {
        {true, "123", 0},
        {false, "lang", res? &res->image("kbd_lang") : 0}
    };
    ...
    Ololo ks2[] = {
        {true, ".", 0},
        {true, ",", 0},
        {false, "ok", res? &res->image("kbd_ok") : 0},
        {false, "hide", res? &res->image("kbd_hide") : 0}
    };
