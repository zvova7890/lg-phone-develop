...
{
    return glStringEngineDrawString(ctx, fti, (const void*)str, x1, y1, x2, y2, align, leter_step,
                       max_leter_cnt, ucs_conv, ctx->pen);
}


inline unsigned int glUCSStringMetrics(fte_info *fti, const wchar_t *str, ft_metrics *metr, int32_t max_width, int flags, int32_t max_leter_cnt)
{
    return ftStringMetrics(fti, (const void*)str, metr, max_width, max_leter_cnt, flags, ucs_conv);
}
...
#ifndef __SYSTEM_FONT__
inline unsigned int glUTF8StringMetrics(fte_info *fti, const char *str, ft_metrics *metr, int32_t max_width, int flags, int32_t max_leter_cnt)
{
    return ftStringMetrics(fti, (const void*)str, metr, max_width, max_leter_cnt, flags, utf8_to_ucs2);
}
...
int glStringMetrics(int font, const char *str, ft_metrics *metr, int32_t max_width, int flags, int32_t max_leter_cnt)
{
    ...
    fte_info *fti = getFont(font);
    if(fti)
        return glUTF8StringMetrics(fti, str, metr, max_width, flags, max_leter_cnt);
    return -1;
#else
