         ...
         int max_leter_cnt, int type, GLColor color, wchar_t (*conv)(const void **))
{
    ...
    int _y1 = y1;
    int max_h;
    int cr_break = type & FT_TEXT_CRBREAK;
    // Fixup leftover position
    _x1 -= fte_get_symbol(fti, letter)->left;

    while(letter && max_leter_cnt > 0)
    {
        if(cr_break && (letter == L'\r' || letter == L'\n')) {
            wchar_t last_l = letter;

            _y1 += fti->h;
            _x1 = x1;
            letter = conv((void *)&str);

            if(last_l == L'\r' && letter == L'\n')
                letter = conv((void *)&str);
...
uint16_t _glStringEngineDrawString(GLContext *ctx, fte_info *fti, const void *str, int x1, int y1,
                            int x2, int y2, int type, uint8_t leter_step, int32_t max_leter_cnt, wchar_t (*conv)(const void **),
                            unsigned int color)
{
    TextLine lines[128];
    ...
    int tmax_height = 0;
    int sentence_hstep = 1;
    ft_metrics metr = {0};


    const void *mystr = str;
    do
    {
        int w = ftStringMetrics(fti, mystr, &metr, INT_MAX, max_leter_cnt, type, conv);
        lines[line].str = mystr;
        lines[line].count = metr.charscount;
        lines[line].width = w;
        lines[line].height = metr.height;
        lines[line].top = metr.top;


        tmax_height += /*(metr.height - metr.top)*/ fti->h + sentence_hstep;
        line++;

        mystr = metr.str;

    }while(!metr.strend && line < 128);




    int s_maxh = tmax_height;
    int max_height = y2-y1;
    int max_width = x2-x1;

    if( type & FT_TEXT_H_UP ) {
        y1 += lines[0].height - lines[0].top;
    }
    else if( type & FT_TEXT_H_CENTER ) {

        if(!(type & FT_TEXT_CENTER_BY_YSTART) || s_maxh < max_height) {
            y1 = y1 + (((max_height/2) - (s_maxh/2))) + (lines[0].height - lines[0].top);
        }
    }
    else if( type & FT_TEXT_H_DOWN ) {
        y1 += max_height - s_maxh + (lines[line-1].height - lines[line-1].top);
    }
    ...
    for(int i=0; i<line; ++i)
    {
        int _x1 = x1;
        if( type & FT_TEXT_W_CENTER ) {
            if( !(type & FT_TEXT_CENTER_BY_XSTART) || lines[i].width < max_width ) {
                    _x1 = x1 + ((max_width/2) - (lines[i].width/2));
            }

        }
        else if( type & FT_TEXT_W_RIGHT ) {
            _x1 = x1+(max_width - lines[i].width);
        }

        glDrawStringHelper(ctx, fti, lines[i].str, _x1, y1+h, x2, y2,
                           leter_step, lines[i].count, type, color, conv);



        h += /*(lines[i].height - lines[i].top)*/ fti->h + sentence_hstep;
    }
...
uint16_t glStringEngineDrawString(GLContext *ctx, fte_info *fti, const void *str, int x1, int y1,
                            int x2, int y2, int type, uint8_t leter_step, int32_t max_leter_cnt, wchar_t (*conv)(const void **),
                            unsigned int color)
{

    if(type & FT_TEXT_CRBREAK ||
            type & FT_TEXT_WORDBREAK ||
            type & FT_TEXT_SENTENCEBREAK)
        return _glStringEngineDrawString(ctx, fti, str, x1, y1, x2, y2, type, leter_step, max_leter_cnt, conv, color);
...
#define metre() if(!metr.str) { \
        s_maxw = ftStringMetrics(font_ctx, str, &metr, INT_MAX, max_leter_cnt, type, conv); \
        if( !(type & FT_TEXT_NOCALC_HCENTER) ) \
            s_maxh = metr.height; \
    }
